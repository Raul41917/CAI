...
def add_agents(builder, condition, task_type, name, folder):
    ...
    for team in range(nr_teams):
        ...
        for agent_nr in range(nr_agents):
            if task_type=="official":
                # Use official agent
                brain = ALWAYS_TRUST(slowdown=8, condition=condition, name=name, folder=folder) # Slowdown makes the agent a bit slower, do not change value during evaluations
                loc = (22,11)
            if task_type=="tutorial":
